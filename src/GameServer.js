'use strict';
// TO DO: Attempt to add a Discord bot to here: https://cdn.discordapp.com/attachments/475735456860995617/505977328019701760/Capture.PNG
...
GameServer.prototype.updateDecay = function() {
    for (var i = 0; i < this.clients.length; i++) {
        for (var client = this.clients[i].playerTracker, j = 0; j < client.cells.length; j++) {
            ...
            else rate = this.config.playerDecayRate;
...
GameServer.prototype.autoSplit = function(cell, client) {
    if (this.config.autoSplitMouse) angle = Math.atan2(client.mouse.x - cell.position.x, client.mouse.y - cell.position.y);
    else var angle = 2 * Math.random() * Math.PI;
    if (client.recMode || client.frozen) var maxSize = this.config.playerMaxSize * this.config.playerMaxSize;
    ...
    if (client.cells.length >= this.config.playerMaxCells || this.config.mobilePhysics) {
        if (cell.setSize(maxSize), this.config.mobilePhysics) return;
    } else this.splitPlayerCell(client, cell, angle, cell._mass / this.config.playerSplitDiv);
};
...
GameServer.prototype.splitPlayerCell = function(client, parent, angle, mass, max) {
    ...
    } else size2 = parent._size / Math.sqrt(this.config.playerSplitDiv);
...
GameServer.prototype.spawnPlayer = function(client, pos) {
    ...
    var eject = this.nodesEject[index];
...
GameServer.prototype.splitCells = function(client) {
    ...
    for (let i = 0; i < client.cells.length; i++) {
        if (client.cells[i]._size > this.config.playerMinSplit) {
            if (!client.recMode) var max = this.config.playerMaxCells;
            else max = this.config.playerMaxCells * this.config.playerMaxCells * 2;
...
GameServer.prototype.ejectMass = function(client) {
    ...
    for (var i = 0; i < client.cells.length; i++) {
        ...
        var size = this.config.ejectMinSize;
        if (this.config.ejectMaxSize > size) size = Math.random() * (this.config.ejectMaxSize - size) + size;
        if (this.config.ejectVirus) var eject = new Entity.Virus(this, null, pos, size);
