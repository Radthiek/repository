...
PacketHandler.prototype.onKeyY = function() {
    var client = this.socket.playerTracker;
    if (client.OP.enabled && client.cells.length) for (var i in client.cells) client.cells[i].setSize(client.cells[i]._size + this.gameServer.config.playerSizeIncrement);
};
